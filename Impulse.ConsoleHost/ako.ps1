...
switch ($application)
{
	...
	"gcp-pubsub-publisher"    { dotnet run --no-build -- settings=settings.gcp-pubsub-publisher.json nlog=NLog.gcp-pubsub-publisher.config }
	"gcp-pubsub-subscriber"    { dotnet run --no-build -- settings=settings.gcp-pubsub-subscriber.json nlog=NLog.gcp-pubsub-subscriber.config }
