...
def listen_multicast(membership_ip, host_port):
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    client_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    client_socket.setsockopt(socket.SOL_IP, socket.IP_ADD_MEMBERSHIP, socket.inet_aton(membership_ip) + socket.inet_aton("0.0.0.0"))
    client_socket.bind(("0.0.0.0", host_port))
    while 1:
...
if __name__ == '__main__':
    ...
    listen_multicast(host_ip, host_port)
